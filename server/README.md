...
- **Passthrough.** When FLAC is requested and the source is a local FLAC file that is at
  most 16-bit, 44.1 kHz and stereo, the original file is served as-is. Its ETag is
  `pt-<size>-<mtime>`.
- **Jobs.** At most 2 encoders run at once. Jobs are de-duplicated per key and run as
  ...
  its job.
- **Cache key and fast hits.** The key is `sha1("v1|<provider instance>|<provider item
  id>|<format>|<mapping details>")[:32]`. The mapping details are the file mtime for
  filesystem providers. Everything in the key comes from the library's provider mapping,
  so a cache hit costs one DB lookup and a `stat`, and never resolves stream details. When
